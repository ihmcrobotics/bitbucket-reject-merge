...
public class RejectMergeCommitHook implements PreReceiveRepositoryHook
{
	...
	public RejectMergeCommitHook(final CommitService commitService,
                                 final GitCommandBuilderFactory commandFactory,
     ...
     */
    private boolean containsIllegalMergeRecursive(final String hash,
     ...
     */
    private boolean isIllegalMerge(final Commit commit,
                                   final Repository repository,
                                   final String branchName,
                                   @Nonnull final HookResponse hookResponse)
    {
        ...
        if(parentBranches.size() < 2)
        {
            ...
            hookResponse.err().println(String.format(
                    "%s: %s",
                    i18nService.getText("com.carolynvs.stash.plugin.reject_merge_commit_hook.rejected_merge_commit",
                            "Rejected merge commit: "),
                    commit.getId()));
            return true;
        }
        return false;
